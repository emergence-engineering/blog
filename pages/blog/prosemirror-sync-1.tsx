...
export const article2Metadata: ArticleIntro = {
  title: "Prosemirror Collab with a sync database",
...
**TLDR**: Create a collaborative editor with a communcation layer provided by a sync database
`;

const MD1 = /* language=md */ `
# Things to know
- *ProseMirror*: a framework / toolkit with which one can create custom text editors
- *collaborative editor*: A text editor where multiple people can edit the same document, for ex. Google docs
...
- *Sync database*: A database which is capable to syncing up to a remote database, so they both contain the same content eventually.
For ex.: CouchDB, Firestore etc... These are all NoSQL databases ( as far as I know )
...
this can be done with just websockets ( and that's what behind the implementation of most sync databasaes ), but that's usually
...
In this demo there are two editors which have their own PouchDB database instance, which sync up to a third database, which 
belongs to a "server". 
...
sent new steps coming from another user ). In that function sendable steps are calculated by the **prosemirror-collab** module, and if there's any then 
they are written to the database as **ClientSteps**. The ProseMirror view is also updated.

#### Receiving new steps

The **fetchNewStepsClient.ts** contains a function which is used in a react **useEffect** in **index.ts**, and gets reloaded every time the documents version is updated.
...
new ProseMirror transaction is sent to the ProseMirror view, which contains all the necessary inforamation to both update the view and the collab state.
