...
export const article19Metadata: ArticleIntro = {
  title: "Stress Test: Lexical / ProseMirror",
  author: "Kata",
  authorLink: null,
...
# The goal
The purpose of this test is to see if there's any difference between the two editors when loaded with text. I mean massive amounts of text. How far they can go, how they handle leaks, how effectively they use memory.
...
## Note
First of all, I couldn't run the test on the two editors for the same number of repetitions - the Lexical editor stopped working after 60 thousand repetitions (somewhere between 60900 and 61000), the ProseMirror editor could run up to 90 thousand repetitions before it shut down. \**It could be because of ….\**
...
\**ProseMirror\**: The total amount of memory initially allocated is used up quite early, as we can see in the total size graph. On both graphs, total memory and memory usage seem to oscillate within a certain range, the latter with an upper boundary at around 30,000,000 bytes and a lower boundary around 10,000,000 bytes. The editor may be intermittently freeing memory or performing periodic optimizations. This could be a sign of efficient garbage collection and memory management strategies. Also, the gradual increase in memory usage suggests a more predictable and controlled memory usage pattern.
...
The presence of garbage collection might mean that the ProseMirror editor could handle long-term usage better than the Lexical editor, as it doesn't allow the heap size to grow unbounded. However, the constant fluctuation could also mean that the garbage collection process itself might be impacting performance if it's too frequent or intense.
