---
name: web-design
description: Rules for building or changing any UI on the Emergence Engineering website. Load before any frontend work: new pages, sections, components, styling, layout, or matching a reference design. Covers the two design systems, brand tokens, anti-generic guardrails, accessibility and performance floor, and the mandatory screenshot review loop.
---
...
- **Invoke the `frontend-design` skill** before writing or changing any UI. No exceptions.
- Decide which mode you are in: **Reference match** or **Design from scratch**. They have different rules.
...
- Hit targets at least 24px, 44px on touch. Inputs at least 16px font on mobile. Never disable browser zoom.
...
- Forms: errors inline next to the field, focus the first error on submit, correct `autocomplete`, `name`, `type`, and `inputmode`, never block paste.
- Brand and product names get `translate="no"` so auto-translate does not mangle them on the bilingual site.
