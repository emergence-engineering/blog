...
const placeholderEmitter = (
  ...
  if (
    pos.pos &&
    view.dragging &&
    sliceTypes.includes(view.dragging.slice.content.content[0].type.name)
  ) {
    const { tr } = view.state;
 ...
 */
function dropCursor(
  DOMPlaceHolders: Record<
    string,
    (node: Node, depth: number) => string | HTMLElement
  >,
  containerValidation: (
    parent: PMNode,
    depth: number,
    ...
    state: {
      ...
      apply(tr, set, state) {
        ...
        if (dropPoint || dropPoint === 0) {
          ...
          const elementToDepth = DOMPlaceHolders[action.slice.type.name](
            action.slice,
            container.depth,
          );
          let placeholderDOM;
          ...
          if (typeof elementToDepth === "string") {
            ...
            placeholderDOM = DOMSerializer.fromSchema(
              state.schema,
            ).serializeNode(action.slice);
            if (placeholderDOM instanceof Element)
              placeholderDOM.className = elementToDepth;
          } else {
...
class DropCursorView {
  ...
  updateOverlay() {
    ...
    if (
      ...
      (this.editorView.dragging &&
        // @ts-ignore
        this.blockNodes.includes(
          this.editorView.dragging.slice.content.content[0].type.name,
        )) ||
      !this.editorView.dragging
    )
      return;
    ...
    if (!$pos.parent.inlineContent) {
      ...
      if ((before || after) && this.cursorPos !== null) {
        const nodeDOM = this.editorView?.nodeDOM(
          this.cursorPos - (before ? before.nodeSize : 0),
        );
        const nodeRect =
          nodeDOM instanceof HTMLElement && nodeDOM?.getBoundingClientRect();
        if (!nodeRect) return;
  ...
  dragstart(event: DragEvent) {
    ...
    // @ts-ignore
    const draggedNode: PMNode =
      this.editorView.dragging?.slice.content.content[0];
    ...
    if (
      ...
      event.target?.dataset.svgHandle
    ) {
    ...
    if (ghost instanceof HTMLElement) {
      ...
      if (this.editorView.dom instanceof HTMLElement)
        this.editorView.dom.style.caretColor = "transparent";
