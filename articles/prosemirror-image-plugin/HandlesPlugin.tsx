...
const findParentNodeWithSomeTypeClosestToPos = (
  ...
  const result = types.map((type) => {
    ...
    if (type.name === TOGGLE_LIST_ITEM && parent) {
      const insideToggleBody = findParentNodeOfTypeClosestToPos(
        position,
        schema.nodes[TOGGLE_ITEM_CONTENT],
      );
      const onToggleTitle = findParentNodeOfTypeClosestToPos(
        position,
        schema.nodes[TOGGLE_ITEM_TITLE],
      );
...
export function handlesFactory(
  isBlock = false,
  onMouseDown?: () => void,
  onMouseUp?: () => void,
) {
