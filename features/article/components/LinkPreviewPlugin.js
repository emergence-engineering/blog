...
function isPreviousNodeValid(node) {
  ...
  return (
    previousNode === null ||
    lexical.$isLineBreakNode(previousNode) ||
    (lexical.$isTextNode(previousNode) &&
...
function isNextNodeValid(node) {
  ...
  return (
    nextNode === null ||
    lexical.$isLineBreakNode(nextNode) ||
    (lexical.$isTextNode(nextNode) &&
...
function handleLinkCreation(node, matchers, onChange) {
  ...
  while ((match = findFirstMatch(text, matchers)) && match !== null) {
    ...
    if (isValid) {
      ...
      const linkNode = link.$createLinkPreviewNode(match.url, match.attributes);
...
function handleBadNeighbors(textNode, matchers, onChange) {
  ...
  if (link.$isLinkPreviewNode(previousSibling) && !startsWithSeparator(text)) {
    previousSibling.append(textNode);
    handleLinkEdit(previousSibling, matchers, onChange);
    onChange(null, previousSibling.getURL());
  }

  if (link.$isLinkPreviewNode(nextSibling) && !endsWithSeparator(text)) {
...
function useAutoLink(editor, matchers, onChange) {
  react.useEffect(() => {
    // if (!editor.hasNodes([link.LinkPreviewNode])) {
    //   {
    //     throw Error(
    //       `LexicalAutoLinkPlugin: LinkPreviewNode not registered on editor`,
    //     );
    //   }
    // }
...
function LinkPreviewPlugin({ matchers, onChange }) {
